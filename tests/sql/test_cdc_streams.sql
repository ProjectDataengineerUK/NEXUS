...
SELECT 'AT-124' AS test_id,
       ...
       CASE
           WHEN COUNT(*) = 9 THEN 'PASS'
           ELSE 'FAIL: apenas ' || COUNT(*)::VARCHAR || '/9 tasks de CDC encontradas'
       END AS result
FROM INFORMATION_SCHEMA.TASKS
WHERE TASK_SCHEMA = 'CORE'
  AND TASK_NAME IN (
      'TASK_MERGE_SAP_CUSTOMERS', 'TASK_MERGE_SAP_INVOICES', 'TASK_MERGE_SAP_ORDERS',
      'TASK_MERGE_ORACLE_CUSTOMERS', 'TASK_MERGE_ORACLE_ORDERS', 'TASK_MERGE_ORACLE_INVOICES',
      'TASK_MERGE_HUBSPOT_CONTACTS', 'TASK_MERGE_HUBSPOT_DEALS', 'TASK_MERGE_HUBSPOT_COMPANIES'
  );
...
SELECT 'AT-124b' AS test_id,
       ...
       CASE
           WHEN COUNT(*) = 9 THEN 'PASS'
           ELSE 'FAIL: ' || COUNT(*)::VARCHAR || '/9 tasks usam NEXUS_COMPUTE_WH'
       END AS result
FROM INFORMATION_SCHEMA.TASKS
WHERE TASK_SCHEMA = 'CORE'
  AND TASK_NAME IN (
